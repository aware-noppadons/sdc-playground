---
name: SDC agent task
about: File work for an autonomous SDC agent (apply the agent-ready label to start)
title: ""
labels: agent-ready
---

<!--
  SDC agent-task issue template — GitHub variant (CANONICAL COPY).

  Deploy into each project repo as:  .github/ISSUE_TEMPLATE/sdc-agent-task.md
  (the sdc- prefix avoids clashing with a project's own templates)
  so it appears in GitHub's "Choose a template" picker when opening an issue.
  The YAML front-matter above pre-applies the `agent-ready` label; remove it
  while drafting if you are not ready for the agent to pick the issue up.
...
<!-- base-branch: <branch>   (optional — fork this issue's working branch FROM, and target its PR AT, this branch instead of the project default. Use it to stack multiple issues on a shared feature branch. The branch must already exist on the remote, or the issue is parked agent-blocked. Single-repo only — ignored when a "Repositories / branches" section is present. Omit/comment → the project default branch, unchanged.) -->
...
<!-- ## Phases   (OPTIONAL, single-repo only — delete this whole section if the issue runs as one pass.)
     ...
     OWN Claude run on the SAME branch, in order — commits accumulate, ONE PR is
